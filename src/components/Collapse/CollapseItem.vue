<template>
  ...
  >
    <!-- 标题部分 -->
    <div
      class="ZH-collapse-item__header"
      :class="{
        'is-disabled': disabled,
        // isActive 并非父组件传递过来的
    ...
    >
      ...
      <!-- 右侧的图标 -->
      <ZH-Icon icon="angle-right" class="header-angle" />
    </div>
    <!-- 内容部分 -->
    <Transition name="slide" v-on="transitionEvents">
      <div class="ZH-collapse-item__wrapper" v-show="isActive">
        <div class="ZH-collapse-item__content" :id="`item-content-${title}`">
          <slot />
        </div>
      </div>
    </Transition>
